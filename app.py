...
# ---------- STYLE (orange buttons + clean cards) ----------
...
st.sidebar.caption("AKG yang dihitung per porsi makan (~1/3 Kebutuhan Harian)")
...
# =====================================================
# NUTRIENT RANGE — DO NOT CHANGE KEYS
# =====================================================
...
def compute_confidence(vec):
    spread = np.std(vec)
    return round(float(1 / (1 + np.exp(-spread * 3))) * 100, 1)
...
def plot_bar(akg):
    ...
    return px.bar(df, x="Nutrient", y="AKG %", text="AKG %", height=360)
...
if uploaded:
    ...
    st.image(img_display, caption="Preview (224x224)", use_container_width=False)
    if st.button("Analisis Gizi", use_container_width=True):
        ...
        nutrisi = calibrated_output(vector_to_dict(vec))
        conf = compute_confidence(vec)

        akg_daily = compute_akg_percent(nutrisi, age_group)
        akg = {k: round(v / 0.33, 1) for k, v in akg_daily.items()}

        # GradCAM
        cam_model = SimpleGradCAM(get_encoder_model(), get_target_layer())
...
if "nutrisi" in st.session_state:
    ...
    with c3:
        ...
        st.metric("Model Confidence", f"{st.session_state.conf}%")
        for k, v in nutrisi.items():
            st.write(DISPLAY_NAME.get(k, k), v)

    st.plotly_chart(plot_radar(nutrisi), use_container_width=True)
    st.plotly_chart(plot_bar(akg), use_container_width=True)

    st.subheader("Rekomendasi Gizi Personal")

    recs = build_recommendation(akg)
    for r in recs:
        st.write("•", translate_rec_text(r))
    ...
    if st.button("Buat Laporan PDF"):
        ...
        path = build_pdf_report(child, nutrisi, akg, recs)
...
if st.session_state.history:
    ...
    sel = st.sidebar.selectbox("Open previous result", labels)
